...
const DEBUG = false;
function logDebug(...args){ if (DEBUG) console.debug('[CipherGuard]', ...args); }
function logWarn(...args){ if (DEBUG) console.warn('[CipherGuard]', ...args); }
function logError(...args){ if (DEBUG) console.error('[CipherGuard]', ...args); }
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  chrome.contextMenus.create({ id: 'cipherguard_paste_username', title: 'CipherGuard: Paste username', contexts: ['editable'] });
  chrome.contextMenus.create({ id: 'cipherguard_paste_password', title: 'CipherGuard: Paste password', contexts: ['editable'] });
  chrome.contextMenus.create({ id: 'cipherguard_autofill', title: 'CipherGuard: Autofill login', contexts: ['page', 'frame', 'editable'] });
    });
  } catch (_) {}
});
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if (info.menuItemId === 'cipherguard_paste_username') {
  ...
  } else if (info.menuItemId === 'cipherguard_paste_password') {
      await chrome.scripting.executeScript({
        target: { tabId: tab.id, frameIds: info.frameId ? [info.frameId] : undefined, allFrames: false },
        func: (value) => {
          const el = document.activeElement;
  ...
  } else if (info.menuItemId === 'cipherguard_autofill') {
