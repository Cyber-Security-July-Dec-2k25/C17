// content.js - robust autofill for CipherGuard
...
(function () {
  ...
  function log(...args){ if (DEBUG) console.debug('[CipherGuard]', ...args); }
  ...
  user.dataset.cipherguardApplied = '1';
    }
    setValue(pw, cred.password || '');
  pw.dataset.cipherguardApplied = '1';
  }
  ...
  if ((fields.user && fields.user.dataset.cipherguardEdited === '1') || (fields.pw && fields.pw.dataset.cipherguardEdited === '1')) {
      log('skip autofill due to user edits');
      return;
    }
    // Avoid repeatedly reapplying if we've already applied once and the field has a value
  if (fields.pw && fields.pw.dataset.cipherguardApplied === '1' && fields.pw.value) {
  ...
  if (!form.__cipherguard_hooked) {
  form.__cipherguard_hooked = true;
